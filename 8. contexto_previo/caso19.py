...
def caso_19_derivada_regla_cadena():

    # Caso base:
    funcion = "ln(sin(x))"
    contexto = "ten en cuenta la regla de la cadena: f(g(x))' = f'(g(x)) * g'(x)"
    prompt_base = (
        f"Calcula la derivada de {funcion}."
        "Responde SOLO con la derivada, sin texto adicional y en formato ASCII."
    )

    # Caso transformado:
    prompt_transformado = (
        f"Calcula la derivada de {funcion}, {contexto}."
        "Responde SOLO con la derivada, sin texto adicional y en formato ASCII."
    )
