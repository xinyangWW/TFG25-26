...
def caso_12_limite_LHôpital():

    # Caso base:
    expr = "lim_{x->0} (sin(x)/x)"
    contexto = "recuerda la regla de L'Hôpital: lim_{x->a} = f(x)/g(x) = f'(x)/g'(x)"
    prompt_base = (
        f"Calcula el límite {expr}."
        "Responde SOLO con un número, sin texto adicional."
    )

    # Caso transformado:
    prompt_transformado = (
        f"Calcula el límite {expr}, {contexto}."
        "Responde SOLO con un número, sin texto adicional."
    )
